			...
			u32 attr, int chan, long *val)
{
	...
	case hwmon_temp:
		...
		ret = wait_for_completion_interruptible_timeout(&priv->completion, HWMON_TIMEOUT);
		if (ret <= 0) {
			if (!ret) {
				       ...
				       struct socfpga_hwmon_priv *priv)
{
	...
	if (of_property_read_string(child, "name", &type))
		return dev_err_probe(dev, -EINVAL,  "No type for '%s'\n", child->full_name);

	for_each_child_of_node(child, grandchild) {
		ret = of_property_read_u32(grandchild, "reg", &val);
		if (ret)
			return dev_err_probe(dev, ret, "missing reg property of %pOFn\n", grandchild);

		ret = of_property_read_string(grandchild, "label", &label);
		if (ret)
			return dev_err_probe(dev, ret, "missing label propoerty of %pOFn\n", grandchild);
