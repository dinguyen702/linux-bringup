...
struct fcs_priv_data {
	int (*smmu_setup)(struct intel_fcs_priv *priv);
	int (*smmu_teardown)(struct intel_fcs_priv *priv);
};
...
static int agilex_smmu_setup(struct intel_fcs_priv *priv)
{
	struct stratix10_svc_client_msg msg;
	int i, ret;
	struct page *page;
...
static int fcs_driver_probe(struct platform_device *pdev)
{
	...
	struct intel_fcs_priv *priv;
	int ret;
	const struct fcs_priv_data *data;
	...
	if (data)
		ret = data->smmu_setup(priv);

	return ret;
}

static int agilex_smmu_teardown(struct intel_fcs_priv *priv)
{
	struct stratix10_svc_client_msg msg;
	int i, ret;
...
static const struct fcs_priv_data agilex_fcs_data = {
	.smmu_setup = agilex_smmu_setup,
	.smmu_teardown = agilex_smmu_teardown
};

static const struct of_device_id fcs_of_match[] = {
	{.compatible = "intel,stratix10-soc-fcs"},
	{.compatible = "intel,agilex-soc-fcs", .data = &agilex_fcs_data},
	{},
};
